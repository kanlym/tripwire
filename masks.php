...
$name = isset($_REQUEST['name']) ? xssafe($_REQUEST['name']) : null;
...
if ($mode == 'search') {
	...
	if (!$name) {
		...
		if (in_array($ownerID.'_'.$ownerType, $adds)) {
			...
			if ($stmt->execute()) {
				...
				// Log create
				$query = 'INSERT INTO _history_masks (maskID, name, ownerID, ownerType, adds, deletes, requested, requestedBy, mode)
							VALUES (:maskID, :name, :ownerID, :ownerType, :adds, :deletes, NOW(), :requestedBy, :mode)';
				$stmt = $mysql->prepare($query);
				$stmt->bindValue(':maskID', $mask, PDO::PARAM_INT);
				$stmt->bindValue(':name', $name, PDO::PARAM_STR);
				$stmt->bindValue(':ownerID', $ownerID, PDO::PARAM_INT);
				$stmt->bindValue(':ownerType', $ownerType, PDO::PARAM_INT);
				$stmt->bindValue(':adds', json_encode($adds), PDO::PARAM_STR);
				$stmt->bindValue(':deletes', json_encode($deletes), PDO::PARAM_STR);
				$stmt->bindValue(':requestedBy', $userID, PDO::PARAM_INT);
	...
	// Log save
	$query = 'INSERT INTO _history_masks (maskID, adds, deletes, requested, requestedBy, mode)
				VALUES (:maskID, :adds, :deletes, NOW(), :requestedBy, :mode)';
	$stmt = $mysql->prepare($query);
	$stmt->bindValue(':maskID', $mask, PDO::PARAM_INT);
	$stmt->bindValue(':adds', json_encode($adds), PDO::PARAM_STR);
	$stmt->bindValue(':deletes', json_encode($deletes), PDO::PARAM_STR);
	...
	// Log delete
	$query = 'INSERT INTO _history_masks (maskID, requested, requestedBy, mode)
				VALUES (:maskID, NOW(), :requestedBy, :mode)';
	$stmt = $mysql->prepare($query);
	$stmt->bindValue(':maskID', $mask, PDO::PARAM_INT);
	$stmt->bindValue(':requestedBy', $userID, PDO::PARAM_INT);
	$stmt->bindValue(':mode', $mode, PDO::PARAM_STR);
